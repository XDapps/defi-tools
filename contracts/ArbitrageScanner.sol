...
contract ArbitrageScanner is OwnableUpgradeable {
    ...
    function initializeContract(
        ...
        uint8[] memory _dexProtocols,
        address[] memory _dexFactories,
        address[] memory _dexRouters,
        address[] memory _dexQuoters,
        uint24[][] memory _dexFees
    ) public initializer {
        __Ownable_init(_msgSender());
        for (uint256 i = 0; i < _dexIds.length; i++) {
            protocol[_dexIds[i]] = _dexProtocols[i];
            factories[_dexIds[i]] = _dexFactories[i];
            routers[_dexIds[i]] = _dexRouters[i];
            quoters[_dexIds[i]] = _dexQuoters[i];
            dexFees[_dexIds[i]] = _dexFees[i];
        }
    }
